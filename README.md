...
## 1.📚词库内容分类
计划按中医内容分类：（内容动态更新，如资料增多不适合混合管理则会分多个文件单独存放，如穴位文件就分了14正经穴位+经外奇穴+董氏奇穴。可以按需要单独选择，比如只需362穴位的，就只把TCM_standard_362_acupoints.dict.yaml这个文件放进你的输入法包即可）
1. **中医基础 (`TCM_basics.dict.yaml`)**：藏象、气血津液、阴阳五行、诊法等基础理论词汇。
2. **中药方剂 (`TCM_herbs.dict.yaml`)**：单味中药、经典方剂、中成药及古籍药名。
3. **针灸穴位_362 (`TCM_standard_362_acupoints.dict.yaml`)**：十四正经的穴位（362个腧穴及国际标准代号）。
4. **针灸穴位_51 (`TCM_extra_51_acupoints.dict.yaml`)**：经外奇穴（51个未归经的穴位及国际标准代号）。
5. **针灸穴位_dong (`TCM_dong_dong_acupoints.dict.yaml`)**：董氏奇穴（董氏奇穴合集）。
